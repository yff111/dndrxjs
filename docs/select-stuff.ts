...
export const toggle = <T>(list: readonly T[], item: T) => {
  if (!list && !item) return []
  if (!list) return [item]
  if (!item) return [...list]
  const matcher = (x: T) => x === item
  const existing = list.find(matcher)
  if (existing) return list.filter((x) => !matcher(x))
  return [...list, item]
}

const toggleAllInbetween = (
  selected: string[],
  all: string[],
  startId: string,
  endId: string,
...
export const useSelectStuff = (
  container: HTMLElement,
  onUpdate: (selected: string[]) => any,
  selector: string = "[data-id]",
  getIdFromFn: (element: Element) => string = (element: Element) =>
  ...
  const toggleSelectAll = () => (
    (selected = areAllItemsSelected() ? [] : getAll()), update()
  )

  const selectAll = () => ((selected = getAll()), update())
  const reset = (newValue: string[] = []) => ((selected = newValue), update())
